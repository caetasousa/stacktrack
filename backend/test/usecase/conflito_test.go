...
func TestSegundaEdicaoComVersaoVelhaDa409(t *testing.T) {
	...
	antes, _ := q.cards.BuscarPorID(cardID)
	versaoQueOsDoisViram := antes.Version

	if _, err := q.card.Editar(cardID, "ana", "Texto da ana", "", "", versaoQueOsDoisViram); err != nil {
		t.Fatalf("a primeira edição devia passar: %v", err)
	}

	_, err := q.card.Editar(cardID, "bob", "Texto do bob", "", "", versaoQueOsDoisViram)
	...
	depois, _ := q.cards.BuscarPorID(cardID)
...
func TestSemVersaoInformadaNaoHaConferencia(t *testing.T) {
	...
	if _, err := q.card.Editar(cardID, "ana", "Primeira", "", "", 0); err != nil {
		t.Fatalf("erro inesperado: %v", err)
	}
	if _, err := q.card.Editar(cardID, "ana", "Segunda", "", "", 0); err != nil {
...
func TestCadaEdicaoIncrementaAVersao(t *testing.T) {
	...
	inicial, _ := q.cards.BuscarPorID(cardID)
	for i := 0; i < 3; i++ {
		atual, _ := q.cards.BuscarPorID(cardID)
		if _, err := q.card.Editar(cardID, "ana", "Texto", "", "", atual.Version); err != nil {
...
func TestORepositorioRecusaEscritaComVersaoDefasada(t *testing.T) {
	...
	// juntas teriam.
	copiaAna, _ := q.cards.BuscarPorID(cardID)
	copiaBob, _ := q.cards.BuscarPorID(cardID)

	if err := copiaAna.Editar("Da ana", "", ""); err != nil {
		t.Fatalf("erro inesperado: %v", err)
	}
	if err := q.cards.Atualizar(copiaAna); err != nil {
	...
	if err := q.cards.Atualizar(copiaBob); !errors.Is(err, dcard.ErrConflito) {
