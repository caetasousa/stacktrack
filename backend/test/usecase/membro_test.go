...
func (c *colaboracao) conta(t *testing.T, nome, email string) string {
	t.Helper()
	out, err := c.cadastro.Executar(ucauth.CadastroInput{Nome: nome, Email: email, Senha: "senha-boa-123"})
	if err != nil {
...
func TestConvidarQuemJaTemContaAdicionaNaHora(t *testing.T) {
	...
	resultado, err := c.membroUC.Convidar(boardID, ana, "bob@exemplo.com", membro.PapelEditor)
	if err != nil {
	...
	vinculo, _ := c.membros.Buscar(boardID, bob)
...
func TestConvidarReconheceContaComOutraCaixaNoEmail(t *testing.T) {
	...
	resultado, err := c.membroUC.Convidar(boardID, ana, "  BOB@Exemplo.com ", membro.PapelLeitor)
	if err != nil {
		t.Fatalf("erro ao convidar: %v", err)
	}
	if !resultado.Adicionado {
...
func TestConvidarQuemNaoTemContaGeraLink(t *testing.T) {
	...
	resultado, err := c.membroUC.Convidar(boardID, ana, "novo@exemplo.com", membro.PapelEditor)
	if err != nil {
		t.Fatalf("erro ao convidar: %v", err)
	}
...
func TestConvitePersisteApenasOHashDoToken(t *testing.T) {
	...
	resultado, _ := c.membroUC.Convidar(boardID, ana, "novo@exemplo.com", membro.PapelEditor)

	if achado, _ := c.convites.BuscarPorTokenHash(resultado.Token); achado != nil {
...
func TestSoODonoConvida(t *testing.T) {
	...
	_, err := c.membroUC.Convidar(boardID, bob, "outro@exemplo.com", membro.PapelLeitor)
...
func TestNaoDaParaConvidarQuemJaParticipaNemASiMesmo(t *testing.T) {
	...
	if _, err := c.membroUC.Convidar(boardID, ana, "bob@exemplo.com", membro.PapelLeitor); !errors.Is(err, dconvite.ErrJaEMembro) {
		t.Errorf("erro = %v, esperado ErrJaEMembro", err)
	}
	if _, err := c.membroUC.Convidar(boardID, ana, "ana@exemplo.com", membro.PapelLeitor); !errors.Is(err, dconvite.ErrNaoConvidaODono) {
...
func TestNaoDaParaConvidarDuasVezesOMesmoEmail(t *testing.T) {
	...
	ana := c.conta(t, "Ana", "ana@exemplo.com")
	boardID := c.criarQuadro(t, ana, "Estudos")
	c.membroUC.Convidar(boardID, ana, "novo@exemplo.com", membro.PapelEditor)

	_, err := c.membroUC.Convidar(boardID, ana, "novo@exemplo.com", membro.PapelLeitor)
...
func TestAceitarConviteViraParticipacao(t *testing.T) {
	...
	b, papel, err := c.membroUC.Aceitar(resultado.Token, novo)
	if err != nil {
	...
	vinculo, _ := c.membros.Buscar(boardID, novo)
	if vinculo == nil || vinculo.Papel != membro.PapelEditor {
		t.Errorf("vínculo = %+v, esperado editor", vinculo)
	}
	if _, err := c.quadros.Detalhar(boardID, novo); err != nil {
...
func TestOutraContaNaoAceitaOConviteAlheio(t *testing.T) {
	...
	_, _, err := c.membroUC.Aceitar(resultado.Token, intruso)

	if !errors.Is(err, dconvite.ErrOutroDestinatario) {
		t.Errorf("erro = %v, esperado ErrOutroDestinatario", err)
	}
	if vinculo, _ := c.membros.Buscar(boardID, intruso); vinculo != nil {
...
func TestConviteValeUmaVezSo(t *testing.T) {
	...
	if _, _, err := c.membroUC.Aceitar(resultado.Token, novo); err != nil {
		t.Fatalf("primeira aceitação falhou: %v", err)
	}

	// A pessoa já é membro; o link, porém, não vale de novo para ninguém.
	if _, _, err := c.membroUC.Aceitar(resultado.Token, novo); !errors.Is(err, dconvite.ErrInvalido) {
...
func TestTokenDesconhecidoRespondeComoConviteInvalido(t *testing.T) {
	...
	if _, err := c.membroUC.DetalharConvite("token-inventado"); !errors.Is(err, dconvite.ErrInvalido) {
		t.Errorf("detalhar: erro = %v", err)
	}
	if _, _, err := c.membroUC.Aceitar("token-inventado", novo); !errors.Is(err, dconvite.ErrInvalido) {
...
func TestDetalharConviteNaoExigeSessao(t *testing.T) {
	...
	detalhe, err := c.membroUC.DetalharConvite(resultado.Token)
	if err != nil {
...
func TestListarMembrosTrazTodosComNomeEEmail(t *testing.T) {
	...
	bob := c.conta(t, "Bob", "bob@exemplo.com")
	boardID := c.criarQuadro(t, ana, "Estudos")
	c.membroUC.Convidar(boardID, ana, "bob@exemplo.com", membro.PapelLeitor)

	// Até o leitor enxerga a lista: saber com quem se divide um quadro é parte
	// de participar dele.
	participantes, err := c.membroUC.Listar(boardID, bob)
	if err != nil {
...
func TestSoODonoVeOsConvitesPendentes(t *testing.T) {
	...
	boardID := c.criarQuadro(t, ana, "Estudos")
	c.convidar(t, boardID, bob, membro.PapelEditor)
	c.membroUC.Convidar(boardID, ana, "novo@exemplo.com", membro.PapelLeitor)

	doDono, err := c.membroUC.ListarConvites(boardID, ana)
	if err != nil || len(doDono) != 1 {
		t.Fatalf("o dono devia ver 1 convite: %d, %v", len(doDono), err)
	}
	if _, err := c.membroUC.ListarConvites(boardID, bob); !errors.Is(err, membro.ErrSemPermissao) {
...
func TestRevogarConviteInvalidaOLink(t *testing.T) {
	...
	if err := c.membroUC.RevogarConvite(resultado.Convite.ID, ana); err != nil {
...
func TestAlterarPapelEremoverExigemODono(t *testing.T) {
	...
	if _, err := c.membroUC.AlterarPapel(boardID, bob, ana, membro.PapelLeitor); !errors.Is(err, membro.ErrSemPermissao) {
		t.Errorf("editor não troca papel: %v", err)
	}
	if err := c.membroUC.Remover(boardID, bob, ana); !errors.Is(err, membro.ErrSemPermissao) {
...
func TestDonoPromoveERemove(t *testing.T) {
	...
	p, err := c.membroUC.AlterarPapel(boardID, ana, bob, membro.PapelEditor)
	if err != nil {
	...
	if err := c.membroUC.Remover(boardID, ana, bob); err != nil {
		t.Fatalf("erro ao remover: %v", err)
	}
	if vinculo, _ := c.membros.Buscar(boardID, bob); vinculo != nil {
		t.Error("o vínculo devia ter sumido")
	}
	// e o quadro some da vista de quem saiu
	if _, err := c.quadros.Detalhar(boardID, bob); err == nil {
...
func TestODonoNaoConsegueSeRemoverSozinho(t *testing.T) {
	...
	if err := c.membroUC.Remover(boardID, ana, ana); !errors.Is(err, membro.ErrSemDono) {
		t.Errorf("erro = %v, esperado ErrSemDono", err)
	}
	if _, err := c.membroUC.AlterarPapel(boardID, ana, ana, membro.PapelLeitor); !errors.Is(err, membro.ErrSemDono) {
...
func TestComOutroDonoOPrimeiroPodeSair(t *testing.T) {
	...
	if err := c.membroUC.Remover(boardID, ana, ana); err != nil {
