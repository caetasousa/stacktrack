...
// --- arquivar ---------------------------------------------------------------
//
// Arquivar tira do quadro sem apagar. É a alternativa reversível ao `apagarCard`
// logo acima, que é definitivo e leva junto comentários, checklists e anexos.

/** CardArquivado é um card fora do quadro, na tela de arquivados. */
export interface CardArquivado {
	id: string;
	colunaId: string;
	// Nome da coluna de origem, resolvido pelo servidor: é onde o card cai ao
	// voltar, e a coluna pode ela própria estar arquivada.
	coluna: string;
	titulo: string;
	cor: Cor | '';
	arquivadoEm: string;
...
/** ColunaArquivada é uma coluna fora do quadro. Os cards dela voltam junto. */
export interface ColunaArquivada {
	id: string;
	titulo: string;
	cor: Cor | '';
	arquivadoEm: string;
...
export interface Arquivados {
	cards: CardArquivado[];
	colunas: ColunaArquivada[];
}

/** arquivados lista o que saiu do quadro e pode voltar. Basta participar. */
export function arquivados(boardId: string): Promise<Arquivados> {
	return apiGet<Arquivados>(`/boards/${boardId}/arquivados`);
}

/**
 * arquivarCard tira o card do quadro sem apagá-lo.
 *
 * Responde **409** se ele já estiver arquivado — o que acontece quando outra
 * pessoa arquivou primeiro, e é informação, não falha.
 */
export function arquivarCard(id: string): Promise<Card> {
	return apiPatch<Record<string, never>, Card>(`/cards/${id}/arquivar`, {});
}

/** desarquivarCard devolve o card à coluna e à posição em que estava. */
export function desarquivarCard(id: string): Promise<Card> {
	return apiPatch<Record<string, never>, Card>(`/cards/${id}/desarquivar`, {});
}

/** arquivarColuna tira a coluna do quadro, com os cards dela. */
export function arquivarColuna(id: string): Promise<Coluna> {
	return apiPatch<Record<string, never>, Coluna>(`/colunas/${id}/arquivar`, {});
}

/** desarquivarColuna devolve a coluna ao quadro, com os cards que ela tinha. */
export function desarquivarColuna(id: string): Promise<Coluna> {
	return apiPatch<Record<string, never>, Coluna>(`/colunas/${id}/desarquivar`, {});
}
