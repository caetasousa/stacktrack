...
func (a *ambiente) cadastrar(t *testing.T, nome, email, senha string) *ucauth.SessaoAberta {
	t.Helper()
	out, err := a.cadastro.Executar(context.Background(), ucauth.CadastroInput{Nome: nome, Email: email, Senha: senha})
	if err != nil {
...
func TestCadastroPersisteApenasOHashDoToken(t *testing.T) {
	...
	s, _ := a.sessoes.BuscarPorTokenHash(context.Background(), token.Hash(out.Token))
	if s == nil {
...
func TestLoginComEmailInexistenteGastaOMesmoCustoDeVerificacao(t *testing.T) {
	...
	a.hasher.Chamadas = 0
	_, errSenhaErrada := a.login.Executar(context.Background(), ucauth.LoginInput{Email: "ana@exemplo.com", Senha: "chute"})
...
func TestValidarSessaoRecusaTokenDesconhecido(t *testing.T) {
	...
	if _, err := a.validar.Executar(context.Background(), "token-inventado"); !errors.Is(err, ucauth.ErrSessaoInvalida) {
...
func TestValidarSessaoRecusaSessaoVencida(t *testing.T) {
	...
	a.sessoes.Salvar(context.Background(), vencida)

	if _, err := a.validar.Executar(context.Background(), "token-vencido"); !errors.Is(err, ucauth.ErrSessaoInvalida) {
		t.Errorf("erro = %v, esperado ErrSessaoInvalida", err)
	}
}
...
func TestLogoutInvalidaASessaoNoServidor(t *testing.T) {
	...
	if _, err := a.validar.Executar(context.Background(), out.Token); !errors.Is(err, ucauth.ErrSessaoInvalida) {
...
func TestLogoutDeTokenDesconhecidoNaoEErro(t *testing.T) {
	...
	if err := a.logout.Executar(context.Background(), "token-inventado"); err != nil {
...
func TestLogoutNaoDerrubaAsOutrasSessoesDoUsuario(t *testing.T) {
	...
	if err := a.logout.Executar(context.Background(), primeira.Token); err != nil {
		t.Fatalf("logout falhou: %v", err)
	}

	if _, err := a.validar.Executar(context.Background(), segunda.Token); err != nil {
...
func TestPerfilDeUsuarioInexistenteNaoAutentica(t *testing.T) {
	...
	if _, err := a.perfil.Executar(context.Background(), "usuario-que-nao-existe"); !errors.Is(err, ucauth.ErrSessaoInvalida) {
		t.Errorf("erro = %v, esperado ErrSessaoInvalida", err)
	}
}
