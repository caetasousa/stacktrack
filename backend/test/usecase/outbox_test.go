...
func comOutbox(q *quadro) (*escritaAtomicaFalsa, *publicadorEspiao) {
	atomica := &escritaAtomicaFalsa{
		repos: ucboard.Escrita{Cards: q.cards, Colunas: q.colunas, Boards: q.boards},
	}
...
// Etiqueta, checklist e anexo NÃO usam o caminho atômico, e isso é decisão, não
// esquecimento: o evento deles é um aviso de "recarregue o quadro", e perder um
// não deixa buraco perceptível — qualquer evento seguinte, ou a reconexão,
// manda a tela buscar tudo de novo.
func TestEventoDeAvisoNaoPassaPelaEscritaAtomica(t *testing.T) {
	e := novoExtras()
	...
	if atomica.mudancasAplicadas != antes {
		t.Error("a etiqueta passou pela escrita atômica; o caminho dela é o de aviso")
	}
}
